...
const EntityTable = ({
  ...
  // here is new column for persian Title
  const renderPersianTitle: TableCellRendered<Column, 'persianTitle'> = (
    _,
    record,
  ) => {
    let myOptions = [];
    if (record.description) {
      for (let i = 0; i < myData.length; i++) {
        if (myData[i]?.name === record.description) {
          myOptions.push(myData[i])
        }
      }
    }
    return (
      <div>
        {
          record?.description ? '': <p>First Select description</p>
        }
        <div>
        {record?.description && (
          <>
          {
            <select style={{borderRadius: "20px",
             width: "200px", height: "35px", outline: "none"}}>
              {myOptions[0]?.persianTitle?.map(item => {
  ...
  const columns: ColumnsType<Column> = useMemo(
    () => [
      ...
      {
        title: t('persianTitle'),
        dataIndex: 'persianTitle',
        key: 'persianTitle',
        accessor: 'persianTitle',
        ellipsis: true,
        width: 220,
        render: renderPersianTitle,
      },
      {
